...
public class PatientReservationFormController implements Initializable {
    ...
    private void handleSauvegarder() {
        if (validateFields() && checkDoctorAvailability()) {
            try {
                ...
                // Set patient_id (replace with actual patient ID from session)
                reservation.setPatient_id(1); // Hardcoded for testing
    ...
    private boolean validateFields() {
        ...
        else if (txtSubject.getText().length() < 5 || txtSubject.getText().length() > 20 )
            errors.append("- The subject must have a minimum length of 5, a maximum length of 20,\n and must begin with a letter\n");
        else if (!txtSubject.getText().matches("^[a-zA-Z].*"))
            errors.append("- The subject must begin with a letter\n");

        if (dpDate_res.getValue() == null )
            errors.append("- The reservation date is required \n");
        else if (dpDate_res.getValue().isBefore(LocalDate.now()))
            errors.append("- The reservation date cannot be in the past\n");
    ...
    private boolean checkDoctorAvailability() {
        ...
        // Pass doctorId directly instead of constructing a string
        boolean isAvailable = service.isDoctorAvailable(doctorId, java.sql.Date.valueOf(date));
        if (!isAvailable) {
