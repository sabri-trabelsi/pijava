...
public class ReservationService implements IService<Reservation> {
    ...
    public boolean isDoctorAvailable(String selectedDoctor, Date date) {
        String req = "SELECT COUNT(*) FROM reservation WHERE doctor_id = ? AND date_res = ? AND statut != 'REFUSED'";
        try (PreparedStatement ps = cnx.prepareStatement(req)) {
            // Extract doctor ID from the 'selectedDoctor' string (assumes format "Dr. <id>")
            int doctorId = Integer.parseInt(selectedDoctor.replace("Dr. ", "").trim());
            ps.setInt(1, doctorId);
